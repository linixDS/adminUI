...
class MailsClass extends BaseClass
{
    protected $args = null;
    ...
    public function getMails($token, $domain, $client){
        if (!isset($token))
        ...
        try {
            ...
            $query = "SELECT account_id as id,username,name,active,created FROM accounts ";
            $query.= " WHERE domain_id=? AND client_id=? AND account_id IN ";
            $query.= "(SELECT account_id FROM accounts_services WHERE service_id=1) ORDER BY username;";

...
class ForwardsClass extends BaseClass
{
    protected $args = null;
    ...
    public function getForwardsFromMail($token, $mail){
        if (!isset($token))
        ...
        try {
            if (!$sess->IsGlobalAdmin()){
                $query = "SELECT forward_id as id, forwarding as forward FROM mail_forwardings WHERE ";
                $query.= "(account_id=(SELECT account_id FROM accounts WHERE account_id=? AND client_id=?)) AND (alias=0);";
                $sth = $db->prepare($conn, $query);
                $sth->execute([$mail]);
    ...
    public function addForward($token, $alias){
        if (!isset($token))
        ...
        if ((!isset($alias['mail'])) || (!isset($alias['forward'])) )
        ...
        $account_id = $this->getAccountId($db, $conn, $address);

        $query = '';

        try {
            $query = "INSERT INTO mail_forwardings (account_id,address,forwarding,alias) VALUES (:ACCOUNTID,:ALIASNAME,:MAIL,0);";
            $sth = $db->prepare($conn, $query);
            ...
            $retAlias['forward'] = $forward;


            return $this->sendResult(201, $retAlias);
        } catch (Exception $e) {
            if (str_contains($e,'Duplicate entry'))
                $this->sendError(409, "Duplikacja przekazania: Podanany alias  został wcześniej wprowadzony.");    
            else                
    ...
    public function removeForward($token, $alias){
        if (!isset($token))
            return $this->sendError(401, 'Access denied - token');
        if (!isset($alias))
            return $this->sendError(401, 'Access denied - data');  
        ...
        if ((!isset($alias['mail'])) || (!isset($alias['forward'])) )
            return $this->sendError(401, 'Access denied - forward data');
        ...
        try {
            $query = "DELETE FROM  mail_forwardings WHERE (account_id=:ACCOUNTID AND forwarding=:ALIASNAME);";
            $sth = $db->prepare($conn, $query);

            $sth->bindValue(':ALIASNAME', $forward, PDO::PARAM_STR);
            $sth->bindValue(':ACCOUNTID', $account_id, PDO::PARAM_INT);
